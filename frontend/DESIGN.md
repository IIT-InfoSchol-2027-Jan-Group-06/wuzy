...
- A `GlassNavButton` stretched into a `control`-tall rounded pill by its `children`, carrying a `small` white label (semibold by default; pass `bold={false}` for regular weight, `numberOfLines` for a multi-line label). Same frosted chrome as every nav/action circle (blur, glass fill, hairline border). Used on Profile's action row and as the expanded ConnectionCard's "Go to profile" and "Refer" actions (both regular weight). Pass `style` for sizing (e.g. `flex: 1`).
...
- The open card (`expanded`) is the same surface at 240 with an action row 35 below the tags: by default two `control` tall buttons, "Go to profile" (pushes `/profile/{id}`) and a "Refer" button. Both are `GlassPillButton`s, the same frosted glass-nav-bar chrome as Profile's Edit profile button. The popup's avatar tap closes it, and pressing the dimmed overlay outside the card closes it too. Pass `expandedContent` to replace those buttons (the refer screen shows a "Send Request" note instead) and `onReferPress` to wire the refer button.

### ConnectionCardPopup
- `import { ConnectionCardPopup } from '@/components/ConnectionCardPopup';`
- The full-frame dim overlay (`rgba(0,0,0,0.5)` backdrop that bleeds past the safe-area insets so it covers the frame edge to edge, `zIndex`/`elevation` 200) that shows a `ConnectionCard` at 240 centered in the middle of the frame, leaving the frame's own background and layout untouched. Rendered as `Screen`'s `overlay` prop inside the route, not a native `Modal`, so a pushed screen (profile, refer) covers it and going back reveals it still open: the popup never follows the user to another screen. Props: `connection` (null hides it), `onClose` (fires on an outside tap or the avatar), `onProfilePress`, `onReferPress`, and `expandedContent`. The screen owns the open card in its own state (`expandedId`) and renders the popup itself, so a resolved referral flow can close it programmatically.
...
- Bubble: `body` text, radius 16 with a 2 nick. Outgoing `yellowMuted` with bg text; incoming `glassFill` with `glassBorder`. A voice note (bubble carrying `audio_url`) renders `VoiceNoteBubble` instead of text or media. A gifted ticket (bubble carrying `ticket`) renders as a `surface` card instead of the frosted bubble: the event's 120 art, Bebas `title` in yellow, `small` date-time and `caption` venue-location lines on padded `surface`, then a reply-style attachment row (`rgba(255,255,255,0.08)` fill, hairline top border) with the gift line itself (`Gifted you <N> ticket(s) to <title>`, no sender name or handle). The card's top-left corner is rounded to the bubble radius when sent, top-right when received (mirroring the standard bubble via the chrome-derived `ticketTopCorners`), and is capped at 85% width.
...
- VoiceNoteBubble (`components/chat/VoiceNoteBubble.tsx`): rendered by ChatBubble when `audio_url` is present (in place of media and any caption image), a 36 yellow play/pause circle over a 22-bar static waveform with a `m:ss` label; outgoing notes use `bg` accents on the `yellowMuted` bubble, incoming use `wuzy-yellow` on `glassFill`. On finish the note seeks back to 0 and releases itself so a later tap replays. Voice note frames over the socket are the WS `voice_note` type carrying `audio_url` + `duration_ms`; files upload through `/upload/audio` and are fetchable from the public `/uploads` mount. The local DB stores both columns plus the reply block as `reply_json` (chat-db v7, which also stores `msg_type` and `ticket_json` for gifted-ticket messages).

### Swipe-to-reply (`ReplyBubble`, `ReplyPreview`, `ChatBubble` swipe gesture)
- **Gesture**: a rightward `Gesture.Pan` on each bubble (threshold 24dp, `runOnJS(true)`, `activeOffsetX([-24, 24])`): swiping right on your own sent message triggers a reply-to-self, and the same right swipe on a received message replies to its sender. The callback sets a `reply` state in the chat route and opens the `ReplyPreview`.
- **`ReplyBubble` / `ReplyPreview`** (`components/chat/ReplyPreview.tsx`): one shared `ReplyBlock` is the reusable quoted-content layout for both the composing preview and every message that carries a reply. White-tinted row (`rgba(255,255,255,0.08)` fill, 1dp `rgba(255,255,255,0.12)` border, 16 radius) with no accent bar. The semibold yellow label sits on top, then the content below: truncated original text, `mic-outline` + "Voice message (m:ss)" with `formatVoiceTime` for voice-note replies, or a square 53dp thumbnail (1:1, no border radius, touching top and bottom; the block never clips its children, so the thumbnail's top two corners stay flat where the block's own top corners round) for picture replies, with a 25px gap between the content and the thumbnail (content `paddingRight: 5` + thumbnail `marginLeft: 20`); a picture reply to a received message (the quoted sender is not the current user) instead pins the thumbnail to the block's right edge (`marginLeft: 'auto'`), with the caption text flowing up to it. Inside Text A, every reply type keeps a 5px left space (`paddingLeft: 5`), including picture replies with or without a caption. Text A and Text B always stretch to equal width (the shorter one widens to the longer): the stack container (`alignItems: 'stretch'`, `maxWidth: '75%'`) sizes to the wider intrinsic block and stretches both. The two exports differ by the 22dp X close button (`ReplyPreview` only, pushed to the right edge). Tapping Text A (`ReplyBubble` `onPress`) scrolls the thread back to the replied-to message, located by the reply's `ref` key (`from-created_at-text`, the same triple the list keys on), via the inverted `FlatList`'s `scrollToIndex`. The composing preview keeps the full 16 radius and sits between the list and the `MessageBar` with `marginBottom: 5` for an exact 5px gap to the bar.
- **Sent reply stack (A over B)**: a sent message that carries a reply renders as two blocks that share one edge: the quoted snippet (A, `ReplyBubble` with `stacked`) directly above the message bubble (B, `ChatBubble` with `squareTop`), no gap between them, inside a shrink-wrapping container (`alignItems: 'stretch'`, `maxWidth: '75%'`) so both stretch to the *wider* of the two intrinsic widths (capped at 75%), i.e. the narrower block is widened to match. `stacked` squares A's bottom-left and bottom-right corners (0) while A keeps the normal top treatment (incoming `NICK`/`RADIUS`, outgoing `RADIUS`/`NICK`, so the topmost tail corner keeps the standard bubble radius); `squareTop` squares both of B's top corners (0) and B keeps its normal bottom corners, so A's bottom edge and B's top edge sit flush with matching sharp corners. A non-reply message renders as before: one `ChatBubble`, `maxWidth: '75%'`.
- **Label** for a reply block: the original caption text when the replied-to message has both a picture and text; otherwise "You" when replying to your own message, or the original sender's name (falling back to the thread name). Computed by the chat route and passed down. The reply's `from_name` is populated by `buildReply`: the server fills it for group frames, and for a DM whose frame lacks it the route falls back to the resolved peer name, so the snippet label always shows the sender's real name.
- **Persistence**: the `reply` field rides on the `ChatMessage` wire type (`ReplyContext` in `lib/ws.ts`) and the server relays it verbatim, so the receiving device sees the quoted block live. The local cache persists it too: chat-db stores `reply_json` (v6) and `getMessages`/`getPendingMessages` parse it back, so snippets survive leaving and reopening the chat, and offline queued replies re-send intact.
- **Photo sends**: the chat route mirrors its `reply` into an out-of-band slot (`setPendingReply`/`getPendingReply` in `lib/media.ts`, the same pattern as the pending photo) because the pushed camera/gallery flows cannot reach the route's state. `PhotoSendFrame` reads it when sending and passes it to `sendDm`/`sendGroup`, then clears the slot; the chat route clears its own `reply` on the flow's return when the slot is empty, so a sent photo does not leave a stale preview above the bar while an abandoned flow keeps the reply.
...
- Sent photos in ChatBubble follow the bubble's own corner grammar. A picture with no caption gets the standard image radius (`Math.round(RADIUS * 0.75)` = 12) on all four corners. A picture with a caption keeps the bubble's asymmetric top: the two bottom corners are 0 (the caption block connects directly under it). Sent pictures round both top corners `RADIUS` (top-right matches top-left); received pictures keep the mirror (`NICK`/`RADIUS`) so the tail corner stays tight. The bubble's own top corners copy the image's on picture messages, so the frosted chrome never peeks past the photo in a corner "tip". When the message carries a reply (B under the quoted block, `squareTop`), both image top corners drop to 0 so the picture sits flush on the quoted block's bottom edge, for bare pictures and captioned ones alike. Thread history reloads on focus (`useFocusEffect` → `reloadHistory`), so a photo sent in the camera flow appears in the thread on return. The native `ws` backend forwards `media_url` untouched; its push text falls back to "Photo" for media-only messages.
- MessageRow: 48 ringed avatar, name (`body` semibold) over preview (`small` gray), time (`caption`) and unread dot on the right. The preview prefixes the sender: `"<full name> : <preview>"` for the latest received message (the other party's display name, or the member's `from_name` in a group) and `"You : <preview>"` for your own. Preview kinds match the reply spec: text verbatim, `Photo` for a picture, `voice message (m:ss)` for a voice note (`formatVoiceTime`). Built in `app/(tabs)/chat.tsx` from the latest message row (`getThreadSummaries`).
...
- Uploads go to `/upload/audio` (kind `audio`, extensions `m4a`/`mp3`/`webm`), served from the public `/uploads` mount so audio URLs are fetchable without auth. Socket frames of type `voice_note` carry `audio_url` + `duration_ms` with no caption. Gifted tickets ride the WS `ticket` type: the event card (`image_url`, `title`, `venue`, `location`, `start_time`, `quantity`) plus the gift line as `text`. The WS router accepts both known types (`message`, `voice_note`) and `ticket`, forwarding the extra fields verbatim, so the local queue, optimistic append, and history reload all round-trip the same shape.
...
- **RankCard**: no card, a centred column at `itemGap` straight on the page. A 225 box with the rank's badge (`deck[10 + rank_index]`, `resizeMode="contain"`); on a rank-up the old badge fades out (260ms) while the new one fades in and springs up from 40, once, no matter how many ranks were jumped. At Diamond three nested `yellowDim` circles (300 / 250 / 200) pulse slowly behind it. Below, centred at gap 4: rank name (`section` semibold yellow), `{total} XP` (`body` white), an 8 tall gutter-wide `bg-white/10` bar whose yellow fill snaps on mount and eases (500ms) on change, and a gray `small` caption "{n} XP to {next}" or "Max rank". Ranks: Bronze 0, Silver 250, Gold 600, Diamond 1100.
...
- **QuestsSection**: a title row with "Quests" (`section` semibold yellow) on the left and a "Badges {earned} of 15" link on the right (`small` semibold `yellowMuted` with a 16 chevron in the same colour) that pushes `/badges`; then every unfinished quest at `itemGap`, claimable ones first and the rest in catalog order (a finished quest leaves the list; its badge lives on the Badges screen), or a gray `small` "Every quest is done" line when none remain. `ACTIONS` maps a quest key to its pill label and route (Add `/connect`, Refer `/connections`, Group `/new-group`, Explore, Buy `/ticket`, Share `/ticket-vault`, Gift, Edit `/edit-profile`); a quest without one (Daily Streak) shows a disabled "Auto" pill.
...
- **Celebration** (`components/awards/Celebration.tsx`): a full-screen `Modal` (`transparent`, fade, `statusBarTranslucent`) over a `bg` scrim at 0.96 that plays after a successful claim; the Awards route builds it by diffing the dashboard at tap time against the one the claim returns. Centred column at `gap`, gutter `side`. Stage 1: headline in Bebas `display` yellow (the claimed tier's name in caps, "3 FRIENDS", or "BADGE EARNED" when the quest's final tier was claimed) drops in from 24 (350ms back-ease), then at 150ms the badge (200 box, `contain`) or "+{xp} XP" (Bebas `hero` yellow) springs in from 0.4, with nothing drawn behind it. The quest name (`body` white) sits under it, with the tier name and XP in `small` gray only on the badge variant. A confetti blast of 180 pieces (rectangles and a few dots, seven colours) launches from the centre of the screen in waves over the first 450ms, each with its own direction, a speed skewed toward slow so the cloud is dense in the middle and sparse at the edges, its own drag (a hard shove that bleeds off), weight, sideways flutter, tumble (a scaleX flip) and spin, fading only in its last third; lives run 1.3 to 2.1s on one 2.6s Reanimated clock. The popper is the one deliberately multi-coloured thing in the app (yellow, pink, purple, orange, cyan, green, white). "Tap to continue" (`small` gray) fades in at 1400ms and pulses. Stage 2, only on a rank change: "RANK UP", the new rank badge, the rank name (`section` semibold yellow) and a second burst. A tap anywhere (or hardware back) advances, then closes. Reduced motion: no confetti, 200ms fades.
...
- Signup is one question per route under `app/onboarding/`, in order: `index` (Individual / Organization outline pills, the latter disabled), `email`, `name`, `username`, `birthday`, `gender`, `password`, `photo` (a 120 circle preview plus an outline pill that opens the image picker; skippable), `interests`. Answers live in the context exported by `app/onboarding/_layout.tsx` (`useOnboarding()`); the last page posts them all, logs in, uploads the picked avatar if there is one, and lands on `/(tabs)/home`. Text pages put Next directly under the field so it stays reachable above the keyboard; gender and interests push Next to the foot with a `flex: 1` spacer. Email and username are checked against `GET /users/availability` before moving on. Gender options are `PillButton`s (solid when picked), interests are `Chip`s under a `SearchBar`, birthday is the native date picker behind a `FormInput`-shaped row (an `<input type="date">` on web); option lists live in `constants/onboarding-data.ts`.
...
- Connections: `ScreenHeader`, then a `ConnectionList` (a shared component): `SearchBar`, a live count from the backend (`/chat/people`), then a `Wheel` of `ConnectionCard`s (slot 132, gap 12) filling the rest of the screen. Refetches on focus, so a QR scan on the Connect screen appears as a new connection with an incremented count. Tapping a card opens it in a centered popup (an in-screen dimmed overlay that covers the whole frame, not a native Modal): the same card at 240 with its action buttons, centered in the middle of the frame, while the frame's own background and layout stay unchanged. Pressing the dimmed overlay closes it; taps inside the popup never do. Only one card opens at a time, and the wheel keeps scrolling underneath. The overlay stays inside the Connections screen, so navigating away (e.g. "Go to profile") covers it and returning reveals it still open; only a manual outside tap (or the avatar) dismisses it.
- Refer to a friend (`app/refer-friend.tsx`): pushed from the Connections screen's "Refer" button with the first recipient's `id` and full `name` as params. Same `ScreenHeader` (back returns to Connections) and the same `ConnectionList`, minus the connections count, listing every connection except the referred user and anyone the referred user is already connected to (their mutual follows, refetched on focus via `GET /users/{id}/connections`). Tapping a card opens the same centered popup as Connections, showing a centered "Send Request" pill (a `GlassNavButton`, the same frosted style as the back button, `wuzy-yellow` text) 35 below the tags. Tapping it creates a backend `ReferralRequest` (`POST /referrals` with `{ first_user_id, second_user_id }`, idempotent only while a pending row exists for that sender-pair) introducing the expanded user and this card's user to each other, and the card locks to a `wuzy-yellow` awaiting note: "Awaiting for {first}'s & {second}'s response!" while both are deciding, dropping to "Awaiting for {name}'s response!" (just the one still deciding) once the other has accepted. The lock is driven by the sender's outgoing referrals (`GET /referrals/outgoing`, refetched on focus via `useOutgoingReferrals`); only cards with a pending request show the note. Once both accept the card flashes "Connection made!" for 1 second; a decline flashes "Referral Denied!" for 1 second. Both resolved outcomes are `small` semibold yellow and auto-close the popup after the flash, marking the referral `consumed` server-side (`POST /referrals/{id}/consume`) and remembering it for the session, so reopening the card returns to the "Send Request" pill permanently and can be used to refer the same pair again. The popup closes right after the flash, staying on this screen.
...
- Edit profile (`app/edit-profile.tsx`): pushed from the Profile tab's "Edit profile" pill and the Complete Profile quest's Edit pill. The profile settings screen: avatar with "Change photo" (system picker, uploaded via `uploadImage('avatar')`), full name and bio inputs, interests typed in and kept as removable chips (no duplicates), "Save changes" (`PATCH /users/me`, then the auth user is merged and "Saved!" flashes), Cancel, and a Delete account action behind `DeleteConfirmationModal`. Saving a profile with a name, bio, photo and at least one interest completes the Complete Profile quest server-side.
- Gift a ticket (`app/gift-ticket.tsx`): pushed from the Gift button on Event details with the event `id`. `ScreenHeader`, then the shared `ConnectionList` without its count; tapping any card opens a `ConfirmDialog` ("Gift a ticket?") and confirming pushes `/ticket` in gift mode (`mode=gift`, `to=<user id>`). The ticket frame is the buy frame's clone with the header reading "Gift a ticket": same event card, quantity stepper, and total; its Pay button now calls `POST /tickets/gift` (quantity-aware, returns the gifted rows), ensures the DM conversation exists (`/ws/conversations/{me}/{to}`), and sends a `ticket` WS frame carrying the event card and the gift line `<name> gifted you <N> ticket(s) to <title>` (the button flashes "Gifted!"). The recipient sees the ticket card as a chat message and the tickets in their vault, plus a Gift row on Notifications.
...
- Chat thread: `ChatHeader`, inverted message list with a date `Chip` at the top, an optional `ReplyPreview` pinned directly above the `MessageBar` when replying to a message, and `MessageBar` in normal flow under the list inside a `KeyboardAvoidingView`.
