...
- **When to Use**: The Awards screen's task list (`app/(tabs)/awards.tsx`). Four parent tasks - Attend Live Events, Social Network, Ticket Sharing, Daily Login - each holding an ordered list of subtasks the user completes one at a time.
...
- **Look**: Compact `surface` card, radius 24, padding 16, cards stacked at `itemGap`. One row: a 60 slot with task art on the left; a center column holding the task name (Poppins semibold white), the active subtask name in yellow `caption`, then the progress bar, then the step dots; the right column holds the status control.
