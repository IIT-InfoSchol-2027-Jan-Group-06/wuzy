...
- GalleryPopup (`components/chat/GalleryPopup.tsx`): a full-screen opaque overlay the chat route lifts above the whole thread (attach sheet included, which stays open underneath). The fixed header pairs the standard frosted `GlassNavButton` back button with the `close` icon on the same default tint as every back button (`rgba(244, 196, 0, 0.1)`), and centers `GALLERY` in the CameraHeader style (`section` Poppins semibold `wuzy-yellow`); `top` gutter and `side` padding match every other screen header, and the header sits outside the scrolling body so the X stays fixed. The body gates on photo access (SDK 57 `expo-media-library`, `granularPermissions: ['photo']`) and never hangs on the spinner: denied or undetermined renders a permission card with `Allow access` (re-requests and is awaited) next to an `Open settings` `Chip` (`Linking.openSettings`); a failed fetch renders a `Could not load photos` card with a `Try again` `Chip` (both `Chip`-style, same token styling as the camera frame's gate). Granted, it renders a three-column square grid (12 gap, `side` gutters) of the device's images newest-first via the `Query` API (`MediaType.IMAGE`, `AssetField.CREATION_TIME` descending), thumbnails sourced from each asset's content id. Tapping a photo resolves its file URI, stashes it as the pending photo (`lib/media.ts`), and pushes `/send-gallery` with the thread's `id`/`kind`/`otherUserId`; the picked cell wears a 3px `wuzy-yellow` ring with a check badge until the chat route clears it. The X button and the chat route's focus-return logic keep both the overlay and the sheet up while `/send-gallery` is stacked (selection is the only thing cleared on return), so the flow runs: Attach -> Gallery -> pick -> send -> back -> next pick else X, which closes overlay and attach sheet together.
