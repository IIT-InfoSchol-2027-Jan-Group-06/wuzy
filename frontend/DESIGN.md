...
- **RankCard**: `surface` card, radius 24, padding 16, one row. Left: a 96 box with the rank's badge (`deck[10 + rank_index]`, `resizeMode="contain"`); on a rank-up the old badge fades out (260ms) while the new one fades in and springs up from 40, once, no matter how many ranks were jumped. At Diamond three nested `yellowDim` circles pulse slowly behind the badge. Right: rank name (`section` semibold yellow), `{total} XP` (`body` white), an 8 tall `bg-white/10` bar whose yellow fill snaps on mount and eases (500ms) on change, and a gray `small` caption "{n} XP to {next}" or "Max rank". Ranks: Bronze 0, Silver 250, Gold 600, Diamond 1100.
- **StickerGrid**: "Sticker Board" (`section` semibold yellow) with "{earned} of 15 collected" (`small` gray), then the deck as 5 columns of square `surface` tiles (radius 16, padding 6, gap 8, width from the gutter like `ProfileGrid`). Earned tiles are full colour; locked ones show the art at 0.25 opacity with a 12 lock icon. Slots 0..9 are earned when a completed quest's `badge_id` matches, 10..13 when the rank is reached, 9 and 14 are "Coming soon". Tapping a tile draws a 1px yellow border and names it in the single gray `small` line under the grid ("Social Network", "Rank reward: Silver"). A tile that became earned since the last dashboard springs in from 0.4 once.
- **QuestsSection**: three groups, Social ("Grow your circle"), Events ("Get out there"), Habits ("Show up every day"): `section` semibold yellow title, gray `small` subtitle, cards at `itemGap`, groups at `gap`. `ACTIONS` maps a quest key to its pill label and route (Add `/connect`, Refer `/connections`, Group `/new-group`, Explore, Buy `/ticket`, Share `/ticket-vault`, Gift, Edit); a quest without one (Daily Streak) shows a disabled "Auto" pill.
- **QuestCard**: `surface` card, radius 24, padding 16. Left column at gap 4: name (`body` semibold white), tier line in yellow `caption` ("Tier 2 of 3 · 3 Friends"), a 24 tall `bg-white/10` pill with the yellow fill behind and "{current} / {target} {unit}" centred in `caption` semibold, then one dot per tier (claimed yellow 6, active yellow 14 wide, upcoming white 15%). A completed quest greys and strikes the name, dims the card to 0.5 and shows the last tier full. Right control, one pill shape (`px 16 py 8`, min width 76, `small` semibold): solid yellow Claim when the tier target is met, a spinner in the same pill while the claim is in flight, an outlined yellow action pill otherwise, a muted gray "Done" when complete. A claim pops the pill (spring 1.12 and back) and bursts eight theme-coloured sparkles driven by one 920ms Reanimated clock whose completion ends the burst (no timers). A failed claim shows the backend's message as a yellow `caption` note under the dots for 2s.
...
- Awards (`app/(tabs)/awards.tsx`): `TabHeader`, then `RankCard`, `StickerGrid` and `QuestsSection` at `gap`, all fed by `useQuests`; a yellow spinner until the first dashboard lands.
