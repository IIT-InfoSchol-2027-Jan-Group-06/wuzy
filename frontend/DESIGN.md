...
- **Look**: A fixed 320 tall transparent yellow glass card: `bg-[#FFE783]/10`, `backdrop-blur` (a BlurView on native), `border border-[#FFE783]/20`, `rounded-3xl`, `overflow-hidden`
- **Placement**: Earned stickers cluster in the middle of the board, spread horizontally around its center point (50, 50) and tilted a little, so they read as a centered badge group rather than a scatter. Placement is memoized per sticker count.
...
- **Sticker feel**: Earned stickers are big, 100-130 square (roughly a third of the board), tilted up to `±10deg`, stacked with `zIndex`, and carry a soft drop shadow. No background cards or borders on individual items.
