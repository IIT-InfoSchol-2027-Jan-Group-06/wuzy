...
- **QuestsSection**: one "Quests" title (`section` semibold yellow) with the gray `small` line "Do the thing, claim the reward", then every unfinished quest in catalog order at `itemGap` (a finished quest leaves the list; its badge lives on the Badges screen), or a gray `small` "Every quest is done" line when none remain. `ACTIONS` maps a quest key to its pill label and route (Add `/connect`, Refer `/connections`, Group `/new-group`, Explore, Buy `/ticket`, Share `/ticket-vault`, Gift, Edit `/edit-profile`); a quest without one (Daily Streak) shows a disabled "Auto" pill.
- **QuestCard**: `surface` card, radius 24, padding 16. Left column at gap 4: name (`body` semibold white), tier line in yellow `caption` ("Tier 2 of 3 · 3 Friends"), a 24 tall `bg-white/10` pill with the yellow fill behind and "{current} / {target} {unit}" centred in `caption` semibold. Right control, one pill shape (`px 16 py 8`, min width 76, `small` semibold): solid yellow Claim when the tier target is met, a spinner in the same pill while the claim is in flight, an outlined yellow action pill otherwise. A claim pops the pill (spring 1.12 and back) and opens the `Celebration` overlay. A failed claim shows the backend's message as a yellow `caption` note under the pill for 2s.
