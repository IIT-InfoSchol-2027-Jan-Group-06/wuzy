...
- **Placement**: A seeded random scatter (`randomScatter`): each sticker lands anywhere inside the box margins so it reads as a fun board rather than a grid, and never overlaps another. Placement is memoized per sticker count.
- **Data**: The board shows only the badges the user has earned. It takes an `earned` prop (names of tasks with at least one claimed step, i.e. `claimed_steps > 0`) and renders each one's reward art from `constants/awards-data.ts` (`questArtFor`). The route file (`app/(tabs)/awards.tsx`) owns the quest fetch and passes the earned names down, so the moment a Claim button is tapped the card's sticker joins the board on the next refetch. When nothing is earned yet the card shows a centered muted hint: "Complete a task to earn its badge".
- **Sticker feel**: Each sticker is 52-62 square, tilted up to `±12deg`, stacked with `zIndex`, and carries a soft drop shadow. No background cards or borders on individual items.
...
- **Status control (right)**: In-progress subtasks with an action get an outlined yellow pill: Attend Live Events "Attend" pushes `/event-details`, Social Network "Add" pushes `/connect`, Ticket Sharing "Share" pushes `/ticket-vault`. When the active subtask hits its target the card shows a solid yellow Claim pill. Pressing it triggers a sparkle-pop: a screen-sized copy of the card's sticker (80% of screen width, in a yellow/16 glowing ring with a yellow/60 border, over a `rgba(0,0,0,0.6)` full-screen scrim in a transparent `Modal`) springs in centered, holds a beat, then shrinks and floats up toward the board as it fades, while the pill itself springs outward with a burst of colored particles. After a short beat the card advances to the next subtask. Claiming a subtask that is not yet at the target returns a 400. Once every substep is claimed the card shows an outlined "All Done" badge.
- **Real-time progress**: The route (`app/(tabs)/awards.tsx`) fetches quests on focus and owns the state. The screens that perform the counted action call `apiBumpQuestProgress` (buying a ticket in `event-details.tsx`, sharing one in `ticket-vault.tsx`, confirming a scan in `connect.tsx`). On return, `QuestsSection` re-renders from the fresh fetch and `BadgeGrid` shows new badges, so the bar, counter, and board update the moment the user returns to the Awards screen.
