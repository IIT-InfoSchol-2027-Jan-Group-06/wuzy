...
- **When to Use**: The Awards screen's task list (`app/(tabs)/awards.tsx`). Each of the three tasks - Daily Login, Social Network, Ticket Sharing - is a single one-step line: perform the counted action once, claim it, done. Cards render in a fixed board order (`QuestsSection`): Daily Login first, Ticket Sharing last, with the custom cards (Purchase Ticket, Complete Profile) in between.
- **Import**: `import { QuestCard } from '@/components/awards/QuestCard';` and `import { QuestsSection } from '@/components/awards/QuestsSection';`
- **Data**: `apiGetQuests()` returns each task with one `active_subtask` carrying its own `current_progress`, `target_count`, `progress_unit` and reward, plus `subtask_step` / `subtask_total` (both 1) for the step dots. `active_subtask` is null once that line is claimed, which the card reads as the task done. `apiBumpQuestProgress(id)` counts one performed action (capped at the 1-step target); `apiClaimQuest(id)` claims it and the task is complete. Task header art maps by task name via `badgeImageForAward(awards, name)` in `constants/awards-data.ts`, which resolves the badge picture from the user's persisted personal sticker deck (15 badge ids held on the user row and mirrored on each award as `badge_id`), so every account earns a different set of stickers.
- **Look**: Compact `surface` card, radius 24, padding 16, cards stacked at `itemGap`. One row: a center column holding the task name (Poppins semibold white), the active subtask name in yellow `caption`, then the progress bar, then the step dots; the right column holds the status control. A fully completed task deactivates the card: the task name grays out and gets struck through, and the whole card dims to opacity 0.5.
- **Progress bar**: A 24 tall pill (`bg-white/10`, 11 white semibold text) with the yellow fill absolutely positioned behind it. The counter text - "0 / 1 friend", "1 / 1 tickets" - sits centered inside the bar on top of the fill, so it stays readable at any fill level. The single step dot sits below it: yellow when active, and no remaining dots.
- **Custom cards**: Purchase Ticket and Complete Profile are custom tasks, not quests, so their progress bar is synthesized in `QuestsSection`: Purchase Ticket counts the user's tickets (capped at 5, "Buy 5 tickets to fill the bar"), Complete Profile is a 1-step flip for the 100 XP profile award. Their right-column button is the action pill (Purchase pushes `/ticket`, Complete pushes `/(tabs)/profile`) until complete, then the solid Claim pill. Claiming Purchase Ticket calls `apiClaimTicketAward()` (`POST /tickets/claim-award`, idempotent): it grants the `ticket_purchase` award once at least 5 tickets are bought and errors with "Buy 5 tickets to fill the bar first" otherwise. Claiming Complete Profile calls `apiCompleteProfile()` (`POST /awards/complete-profile`), which grants the `profile_complete` award once. Both then fall through the same claim success path as quests.
- **Status control (right)**: In-progress subtasks with an action get an outlined yellow pill: Social Network "Add" pushes `/connect`, Ticket Sharing "Share" pushes `/ticket-vault`. Daily Login needs no action button: the bar fills automatically each day on app launch via `apiRecordDailyLogin()`. When the active subtask hits its target the card shows a solid yellow Claim pill. Pressing it triggers a sparkle-pop on the pill (a spring and a burst of colored particles); on success the card advances to the next subtask. Claiming a subtask that is not yet at the target returns a 400. Once every substep is claimed the card shows an outlined "All Done" badge, the name grays out and strikes through, and the card dims to opacity 0.5. In the sticker board (`app/(tabs)/awards.tsx`), the badge art sits centered on the board and the XP bar sits directly beneath it. XP is computed client-side from claimed subtasks: each claimed step plus the two custom tasks (Purchase Ticket, Complete Profile) earns a share of a 600 XP max. The badge slides forward on every claimed subtask: it advances through that user's personal deck (`apiGetMyDeck()`, badge ids persisted on the user row), the old badge fading out and the new one sliding up from below. On load the badge snaps straight to its current rank without animating, so only completions made while the screen is open animate. When all tasks are done the bar shows "600 XP — Max rank", the final badge of the user's deck swaps in, and a soft yellow glow pulses behind it. The sticker board is the earned badge itself at the board's size, with no separate board art behind it. The XP counter sits beneath the board.
- **Real-time progress**: The route (`app/(tabs)/awards.tsx`) fetches quests on focus and owns the state. The screens that perform the counted action call `apiBumpQuestProgress` (sharing a ticket in `ticket-vault.tsx`, confirming a scan in `connect.tsx`). On return, `QuestsSection` re-renders from the fresh fetch, so the bar, counter, and status control update the moment the user returns to the Awards screen.
- **Permanent badges**: A task's badge is permanent once earned. Each quest and custom task claim writes an `Award` row (`award_type` = task name) tied to the claiming user and their personal deck's badge id. The profile hero (`app/(tabs)/profile.tsx`, `app/profile/[id].tsx`) lists every non-empty `Award` for that user as an earned sticker: a quest shows only when its whole chain is done (`active_subtask` is null), custom tasks when their award exists. Because the award rows persist, a completed task's badge stays on the profile forever and never resets. Completed cards also stay put: a quest's card crosses out when the backend reports `active_subtask` null, and the custom tasks re-cross themselves from their persisted `Award` rows (`ticket_purchase`, `profile_complete`) on every load, so a finished task stays crossed out across app restarts.
