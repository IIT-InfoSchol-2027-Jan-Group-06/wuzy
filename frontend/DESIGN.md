...
- **`ReplyBubble` / `ReplyPreview`** (`components/chat/ReplyPreview.tsx`): one shared `ReplyBlock` is the reusable quoted-content layout for both the composing preview and every message that carries a reply. White-tinted row (`rgba(255,255,255,0.08)` fill, 1dp `rgba(255,255,255,0.12)` border, 16 radius), a 3dp left accent bar (yellow for outgoing, `rgba(84,82,56,0.35)` otherwise), the semibold yellow label on top, then the content below: truncated original text, `mic-outline` + "Voice message (m:ss)" with `formatVoiceTime` for voice-note replies, or a square 53dp thumbnail (1:1, no rounding, touching top and bottom) for picture replies, with a 10px gap between the content and the thumbnail. The two exports differ by the 22dp X close button (`ReplyPreview` only, pushed to the right edge). The composing preview keeps the full 16 radius and sits between the list and the `MessageBar` with `marginBottom: 5` for an exact 5px gap to the bar.
