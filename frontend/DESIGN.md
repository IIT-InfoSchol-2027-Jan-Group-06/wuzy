...
- **QuestCard**: `surface` card, radius 24, padding 16. Left column at gap 4: name (`body` semibold white), a 24 tall `bg-white/10` pill with the yellow fill behind and "{current} / {target} {unit}" centred in `caption` semibold. Right control, one pill shape (`px 16 py 8`, min width 76, `small` semibold): solid yellow Claim when the tier target is met, a spinner in the same pill while the claim is in flight, an outlined yellow action pill otherwise. A claim pops the pill (spring 1.12 and back) and opens the `Celebration` overlay. A failed claim shows the backend's message as a yellow `caption` note under the pill for 2s.
