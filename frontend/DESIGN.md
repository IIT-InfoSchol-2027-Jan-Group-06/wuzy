...
- A referral entry in the Notifications list: 48 avatar, gray `Referral` label (`caption`) above the sentence "[name] wants to refer you to [name]" (`small`, up to two lines) in white, gray timestamp (`small`) below. Pending entries add Accept/Decline liquid-glass pills (`LiquidGlass`, `control` tall, flex row, `itemGap` 12): Accept tint 0.3, Decline 0.1, both with `small` semibold yellow text. Tapping either opens a `ConfirmDialog` confirmation ("Accept Referral?"/"Decline Referral?") and confirming resolves the referral server-side; resolved entries are then removed from the list. Same spacing and fonts as `UserRow`.
...
- Refer to a friend (`app/refer-friend.tsx`): pushed from the Connections screen's "Refer to a friend" button with the referred user's `id` and full `name` as params. Same `ScreenHeader` (back returns to Connections) and the same `ConnectionList`, minus the connections count, listing every connection except the referred user and anyone the referred user is already connected to (their mutual follows, refetched on focus via `GET /users/{id}/connections`). Expanding a card here shows a centered "Send Request" pill (a `GlassNavButton`, the same frosted style as the back button, `wuzy-yellow` text) 35 below the tags. Tapping it creates a backend `ReferralRequest` (`POST /referrals`, idempotent only while a pending row exists for that combo) and the card locks to the `wuzy-yellow` note "Request sent to {name}, Awaiting Response" (the referred user's name). The lock is driven by the sender's outgoing referrals (`GET /referrals/outgoing`, refetched on focus via `useOutgoingReferrals`); only cards with a pending request show the note. Each card's state comes from its most recent referral: pending notes lock it, an accepted one becomes the row "The referral was approved!" next to a circular `GlassNavButton` QR icon button (same size, shape, glass and tint as the back button), and a declined one flashes "Referral Denied!" (`small` semibold yellow, same as the pending note) for exactly 3 seconds before resolving itself. Once a resolved referral has been seen (Done tapped, or the 3s declined flash) it is marked `consumed` server-side (`POST /referrals/{id}/consume`) and remembered for the session, so the card permanently returns to the "Send Request" pill and can be used to send a brand new referral to the same target. The QR button pushes `referral-qr` with the referral's id and the approving user's id. The declined flow, and the accepted flow after Done, swap the card's content to the Send Request pill, hold it at full size for 2 seconds, then shrink the card via `heightFor` = 132 and navigate back with `router.back()`. Accept/Decline liquid-glass pills (`LiquidGlass`, Accept tint 0.3, Decline 0.1) appear below the sentence in the Notifications tab; tapping either opens a `ConfirmDialog` confirmation first, and confirming resolves the referral server-side (`POST /referrals/{id}/respond`), signals the sender live, and removes the entry from the list. A connection between the parties after acceptance stays deferred.
- Referral QR (`app/referral-qr.tsx`): pushed from the approved card's QR button on the refer screen with `requestId` and `referredId` (the approving user) as params. Looks like Connect but with no camera and no back button: it fetches the approving user's own record and shows their full `display_name` and their own QR (`QrCode`, `https://wuzy.app/profile/{username}`), never the sender's. The toggle bar is replaced by a single "Done" `GlassNavButton` pill sized exactly like the toggle (width `(glass + 30) * 2 + 1`, height `control + 5`) with `wuzy-yellow` `small` semibold text on the back-button glass. Done consumes the referral and pops back to the refer screen, whose card switches to the Send Request pill, holds for 2 seconds, then collapses and returns to Connections.
- Referral delivery: creating a request notifies the referred user (the one whose card was expanded on Connections) with a live frame over the shared WebSocket (`type: "referral"`, delivered by `deliver_live` in `ws.py`, forwarded by `subscribeReferrals` in `lib/ws.ts`) plus a best-effort OS push banner ("{name} wants to refer you to {name}", tapping opens Notifications). Responding notifies the sender back with a `type: "referral_response"` frame (`subscribeReferralResponses`), which the refer screen consumes to update an open card in place; outcomes also persist through the refetch-on-focus pattern, so they are reflected whenever the card is next opened. The Notifications screen shows their inbox referrals (`GET /referrals/inbox`) as `ReferralNotificationCard` rows in the New group, refreshed on focus and on each live frame; only pending rows are shown, and a row disappears once it is resolved.
