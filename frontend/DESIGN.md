...
- **Gesture**: a horizontal `Gesture.Pan` on each bubble (threshold 24dp, `runOnJS(true)`, `activeOffsetX([-24, 24])`): swiping right on your own sent message triggers a reply-to-self; swiping left on a received message replies to the sender. The callbacks set a `reply` state in the chat route and open the `ReplyPreview`.
- **`ReplyBubble` / `ReplyPreview`** (`components/chat/ReplyPreview.tsx`): one shared `ReplyBlock` is the reusable quoted-content layout for both the composing preview and every message that carries a reply. White-tinted row (`rgba(255,255,255,0.08)` fill, 1dp `rgba(255,255,255,0.12)` border, `borderRadius` 16 matching the bubbles), a 3dp left accent bar (yellow for outgoing, `rgba(84,82,56,0.35)` otherwise), the semibold yellow label on top, then the content below: truncated original text, `mic-outline` + "Voice message (m:ss)" with `formatVoiceTime` for voice-note replies, or a square 53dp thumbnail (1:1, no rounding, touching top and bottom) for picture replies. The only difference between the two exports is the 22dp X close button (`ReplyPreview` only); sent snippets use `ReplyBubble` and are width-constrained to `maxWidth: '75%'` aligned to the bubble side. The composing preview sits between the list and the `MessageBar` with `marginBottom: 5` for an exact 5px gap to the bar.
