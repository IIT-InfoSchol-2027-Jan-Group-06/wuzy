...
/** Send frame in the chat camera flow: the captured picture, a caption pill
 * below it, and a Send button centered under the pill. Sends the photo (plus
 * caption in the same bubble) into the thread the attach sheet opened from,
 * then pops the whole camera flow back to the chat. */
export default function CameraSendScreen() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Pop camera, photo-confirm, and live camera back to the chat thread.
      router.dismiss(3);
    } catch (e) {
